...
The Java language is known to be fast, safe, and easy, but Java build tools 
like Maven or Gradle don't always live up to that reputation.
This talk will explore what "could be": where current Java build tools fall
short: in performance, extensibility, 
and IDE experience. We will end with a demonstration of an experimental
build tool "Mill" that makes use of these ideas, proving out the idea that Java
build tooling has the potential to be much faster, safer, and easier than it is today.

--------------------------------------------------------------------------------

## Can Java Build Tools Be Better?
There are three main areas that Java build tools typically fall short:
...
does and how ir can be used to accomplish their goals. 

However, if you try to use the normal
IDE _"jump to definition"_ on this Maven `pom.xml` snippet, it brings you to this:

![IntellijNettyMaven2.png](docs/modules/ROOT/images/comparisons/IntellijNettyMaven2.png)

To be clear, this result is still useful: we now know that `sources` has the type
`java.io.File[]`, it is `required`, and it is documented as `Additional source directories`.
However, the next question a developer asks is always the same: how is this thing used
internally? How do changes in this value end up influencing the behavior of the system,
in this case the build tool?

This is not unique to Maven and it's `pom.xml`; Gradle suffers the same problem. 
...
Although you could guess - it configures the compiler - it is still reasonable to want
to learn more: 
...
While the Gradle example above is in Groovy, the experience with Kotlin is similar.

To be clear, this is _not_ the IDE experience that someone expects from a JVM project! If
I open a random Java file in any Java project, I expect to be able to pull up the documentation
and signature of _any_ identifier in that file with a single keypress:
...
In a typical Java setup, this seamless _jump-to-definition_ and _find usages_ works throughout:
your own code, third-party libraries, standard libraries, and so on. So this experience is
definitely _achievable_, so how come 

### Extensibility 

All build tools are extensible to some degree, but the ease of extension matters. Let's
...
to support. "counting the lines and saving it" is just the "hello world" version of what
can be very deep and intricate customizations.
...
The thing about this "count lines and save it to a file" requirement is that it is 
...
of choice in about 30 seconds. But the build tools like Maven or Gradle make this
trivial task decidedly non-trivial. That's not to say it's _difficult_, but it's
a lot harder than such a simple task should be!

### Performance

The last area to discuss here is performance. Java is a very performant language, and the
Java compiler written in Java is actually very fast. But no existing build tools seem to
...
with all dependencies already downloaded, and with the `~/.m2` cache already populated. In 
isolation that number seems fine. But it's worth asking: how fast _should_ Java code compile?
...
out of it. Java programs need to be kept warm in-memory in a long-lived process so the JVM
...
Right off the bat, you can mouse of the task and see the documentation inherited from 
...
earlier, perhaps slightly simplified. But what's actually notable is what we _don't_
see here: `doLast`, `dependsOn`, `inputs.files`, `outputs.files`, hard-coded references
to paths like `"src/main/java"`. With Mill, you just write the business logic
...
build tools are _usable_, they are definitely not _great_, and are far from what you
...
of tools like Maven or Gradle: 3-6x faster builds, easier and less error-prone 
extensibility with custom logic, and an IDE experience that allows you to navigate your
build logic just as easily as any application code. While there is still room to improve
- e.g. compile speed still falls short compared to running the Java compiler hot in-memory -
it nevertheless shows a significant step up from tools like Maven or Gradle.
